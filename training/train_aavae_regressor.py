...
from gaze_estimation.model import resnet18, decoder18, ProjectionHeadVAERegression, LatentRegressor
...
class TrainRTGENEAAVAE(pl.LightningModule):
    ...
    def __init__(self, hparams, train_subjects, validate_subjects, test_subjects):
        ...
        extract_img_fn = {
            "left": lambda x: (x[0], x[2], x[5]),
            "right": lambda x: (x[1], x[3], x[5])
        }

        self.encoder = resnet18()  # consider adding more backends
        self.projection = ProjectionHeadVAERegression(latent_dim=hparams.latent_dim, regressor_dim=2)
    ...
    def shared_step(self, batch):
        orig_img, augm_img, gaze = self._extract_fn(batch)
        encoding = self.forward(augm_img)
        mu, logvar, mu_r, logvar_r = self.projection(encoding)
        _, _, z = self.sample(mu, logvar)
        reconstruction = self.decoder(z)

        _, _, r = self.sample(mu_r, logvar_r)
        latent_r = self.latent_regressor(r)

        recons_loss = self.gaussian_likelihood(reconstruction, sample=orig_img).mean()
        recons_loss = torch.exp(recons_loss)

        kld = torch.mean(-0.5 * torch.sum(1 + logvar - (mu - latent_r) ** 2 - logvar.exp(), dim=1), dim=0)
        label_loss = ((0.5 * (mu_r - gaze)**2)/torch.exp(logvar_r)) + 0.5 * logvar_r

        loss = label_loss + self.hparams.kld_weight * kld + recons_loss
        result = {"kld_loss": kld,
